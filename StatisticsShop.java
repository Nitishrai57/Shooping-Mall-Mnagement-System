...
public class StatisticsShop {
	...
	int ans= 1;
	private JLabel lblCategoryWise,lblItemWise;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					StatisticsShop window = new StatisticsShop(vishal,getshop);
	 ...
	 */
	public StatisticsShop(String a,String b) {
		vishal =a;
		getshop =b;
	...
	private void initialize() {
		...
		frame.setBounds(100, 100, 833, 900);
		...
		btnShow.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String query ;
				 ...
				 query = "select category,sum(price) from purchasedhistory  where date between ? and ? and shop=? group by category";
				try {
				PreparedStatement pst = connection.prepareStatement(query);
				pst.setString(1, start);
				pst.setString(2, end);
				pst.setString(3, getshop);
				...
				catch(Exception e)
				{
				 ...
				 query = "select name,sum(price) from purchasedhistory  where date between ? and ? and shop=? group by name";
					try {
					PreparedStatement pst = connection.prepareStatement(query);
					pst.setString(1, start);
					pst.setString(2, end);
					...
					scrollPane.setBounds(29, 623, 329, 157);
		...
		lblItemWise.setBounds(100, 584, 138, 15);
		...
		btnBack.setBounds(650, 840, 117, 25);
